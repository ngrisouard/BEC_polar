...
for t in np.arange((nti+1)*dt, T+dt, dt):
    ...
    for ii in range(-nth//2 +1, nth//2 +1):
        ...
        #Fr = np.dot(interp_ker, fr[:, ii-1+nth//2])
        ...
        #jnk = bessel_zeros[abs(ii), :jmodes]
        #W = bessel_zeros[abs(ii), jmodes]
        
        
        '''
        for r_index in range(len(r)):
            denominator = (special.jn(ii+1, jnk)*(jnk**2-(r[r_index]*W/R)**2))
            if abs(ii) > 0:
                denominator[0] = 1/jmodes
            
            fr[r_index,ii-1+nth//2] = np.dot(Fr[0],2*jnk*special.jn(ii, r[r_index]*W/R)/denominator)
        '''
        #print(ii)
