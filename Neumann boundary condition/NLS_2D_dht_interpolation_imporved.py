...
if nti == 0:        #then define all parameters
    ...
    split_NL = 0  #=0.5 for NLS, =1 for LS, =1/3 for NLS with dissipation
    
    if split_NL == 1:
        V = -np.tanh((R-Rad)/np.sqrt(2))
    else:
        V = -0.5
...
############### Initial condition #########################
    ...
    #np.tanh((R-Rad)/np.sqrt(2)) * np.exp(0j*Thet)
    #np.ones(np.shape(Rad))
    wf =  np.ones(np.shape(Rad))* np.exp(0j*Thet)\
    ...
    # plt.pcolor(X, Y, V, cmap=cc.cm.rainbow)#, vmax = z_max, vmin = z_min)
    # plt.title('Potential')
    # plt.colorbar()
    # plt.show()
    plt.figure(figsize = (10, 8))
    ...
    if comp_ker == 1:
        Neu_gain = np.genfromtxt('Neumann_gain.csv', delimiter=',')
        bessel_zeros = np.genfromtxt('dht_neumann.csv', delimiter=',')
        bessel_zeros_d = np.genfromtxt('dht.csv', delimiter=',')

        for ii in range(-nth//2+1, nth//2+1):

            rr, kk, F, B = dht_neumann(R, bessel_zeros, jmodes, ii, bessel_zeros_d, Neu_gain)
            #rr, kk, F, B = dht_derichlet(R, bessel_zeros_d, jmodes, ii)
            save_dict = {'kk':kk, 'rr':rr,'F':F, 'B':B}
            #H,kk,rr,I,KK,RR, h = dht([],R,bessel_zeros_d,jmodes,ii)
...
for t in np.arange((nti+1)*dt, T+dt, dt):
    ...
    for ii in range(-nth//2 +1, nth//2 +1):
        ...
        adht = np.dot(B, Fr)#+Fr[0]*Fi
    
        #inverse dht
        #Fr = idht(adht*np.exp(-1j*0.5*(kk**2)*dt), I, KK, RR)
        Fr = np.dot(F, adht*np.exp(-1j*0.5*(kk[0]**2)*dt)) #+ adht[0]*np.exp(-1j*0.5*(kk[0]**2)*dt)*Bi
        #Fr = np.dot(adht, B)
        ...
        fr_func = interpolate.interp1d(rr[0], Fr, kind=interp_sch, fill_value='extrapolate')
