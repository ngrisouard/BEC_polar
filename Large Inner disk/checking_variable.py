...
def bessel(order, R, x, root):
    
    temp1 = special.jv(order, root * x /R) * special.yv(order, root)
    temp2 = special.jv(order, root ) * special.yv(order, root * x /R)
...
#%%
#order = 40
test = 1*(bessel(ii, R, RR, bessel_zeros[ii,1]) + 1*bessel(ii, R, RR, bessel_zeros[ii,3]) + 1.5*bessel(ii, R, RR, bessel_zeros[ii,10])) #* np.exp(0j*np.pi/2)
#test = np.ones(nr)
test[0] = 0
test[-1] = 0
